...
export class LpgCallGraphProvider extends WebviewProvider {
    ...
    public generateContent(webView: Webview, source: TextEditor | Uri, options: WebviewShowOptions): string {
       ...
       let graph  = new Map<string, ReferenceNodeInfo>();

       const entry: ReferenceNodeInfo = {  
        rules: new Array<string>(),
        terminal: new Array<string>(),
       };
       entry.rules.push("dddd");
       graph.set("test",entry);
        const data = [];
        for (const entry of graph) {
            ...
            for (const ref of entry[1].terminal) {
                references.push(ref);
            }
